...
- [ ] **비밀번호 찾기**: 이메일로 비밀번호 재설정 링크 발송 기능.
...
- [ ] **장바구니 동기화**: 로그인 시 DB에 저장된 장바구니 목록 불러오기.

## 3단계: 미디어 저장소 (Cloud Storage)
> **목표**: 사용자 업로드 이미지 처리
- [ ] **이미지 업로드**: OOTD 작성 시 사진을 Firebase Storage에 업로드.
- [ ] **프로필 사진**: 마이페이지에서 프로필 이미지 변경 기능.
- [ ] **이미지 최적화**: 업로드 시 리사이징 또는 압축 (Extension 활용).

## 4단계: 보안 및 규칙 (Security Rules)
> **목표**: 데이터 무결성 및 권한 관리
- [ ] **Firestore 규칙**: 본인 데이터만 수정 가능하도록 읽기/쓰기 권한 설정.
- [ ] **Storage 규칙**: 인증된 사용자만 이미지 업로드 가능하도록 설정.
