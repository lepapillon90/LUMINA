...
- [x] **소셜 로그인 (Optional)**: Google 로그인 추가 (Login 페이지).
...
- [x] **이미지 최적화**: 업로드 시 Client-side Canvas 압축 적용 (비용 절감).
...
## 5단계: 서버리스 함수 (Cloud Functions) - *Alternative Implemented*
> **목표**: 백엔드 로직 자동화 (Client-side 대안 구현)
- [x] **트리거**: 회원가입 시 환영 이메일 발송 (Client-side EmailJS).
- [x] **집계**: OOTD 좋아요 수 변경 시 Firestore Transaction으로 실시간 집계 구현.
- [x] **관리자 알림**: 새 주문 발생 시 관리자에게 슬랙/이메일 알림 (Client-side EmailJS).
