...
import { Shield, Plus, Edit3, Trash2, UserCheck, UserX, Search, Mail, Key, UserMinus } from 'lucide-react';
import { User, UserPermissions, UserRole } from '../../../types';
import { getAdminUsers, createAdminUser, updateAdminUser, toggleAdminStatus, demoteAdminToUser, sendAdminPasswordReset, getAllUsers, promoteUserToAdmin } from '../../../services/adminService';
...
const AdminAccountManager: React.FC<AdminAccountManagerProps> = ({ user }) => {
    ...
    const handleDemoteAdmin = async () => {
        ...
        try {
            await demoteAdminToUser(deletingUserId, { uid: user.uid, username: user.username });
            setAdminUsers(prev => prev.filter(u => u.uid !== deletingUserId));
            setIsDeleteModalOpen(false);
            setDeletingUserId(null);
            await showAlert('관리자 권한이 해제되어 일반 사용자로 변경되었습니다.', '권한 해제 완료');
        } catch (error) {
            console.error('Error demoting admin:', error);
            await showAlert('관리자 권한 해제에 실패했습니다.', '오류');
        }
    };
    ...
    return (
        <div className="space-y-6">
                            ...
                            filteredUsers.map((adminUser) => (
                                <tr key={adminUser.uid} className="hover:bg-gray-50 transition group">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`px-2 py-1 rounded-sm text-xs font-medium ${adminUser.isActive
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex items-center justify-end gap-2">
                                            ...
                                            <button
                                                ...
                                                className={`transition ${adminUser.isActive
            ...
            <ConfirmModal
                ...
                title="관리자 권한 해제"
                message="정말로 이 관리자의 권한을 해제하시겠습니까? 해당 계정은 일반 사용자로 변경됩니다."
                confirmLabel="권한 해제"
                isDestructive={true}
            />
        </div>
...
const CreateAdminModal: React.FC<CreateAdminModalProps> = ({ onClose, onSuccess, currentUser }) => {
    ...
    useEffect(() => {
        if (mode === 'select') {
            ...
            setFilteredExistingUsers(existingUsers.filter(u =>
                u.email?.toLowerCase().includes(lowerTerm) ||
                u.username?.toLowerCase().includes(lowerTerm) ||
                u.displayName?.toLowerCase().includes(lowerTerm)
            ));
        }
    }, [userSearchTerm, existingUsers, mode]);
    ...
    const fetchExistingUsers = async () => {
        ...
        try {
            ...
            setExistingUsers(nonAdmins);
            setFilteredExistingUsers(nonAdmins);
        } catch (error) {
            console.error('Error fetching users:', error);
            showAlert('사용자 목록을 불러오는데 실패했습니다.', '오류');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleUserSelect = (user: User) => {
        ...
        setFormData(prev => ({
            ...
            // Reset other fields that might not be in user object or relevant
            password: '', // Password not needed for promotion
            displayId: '',
            jobTitle: '',
            department: '',
            permissions: {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (mode === 'create') {
                ...
                await showAlert('관리자로 승격되었습니다.', '알림');
            }
            onSuccess();
        } catch (error: any) {
            console.error('Error processing admin user:', error);
            await showAlert(error?.message || '작업에 실패했습니다.', '오류');
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
            <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
                <div className="flex justify-between items-center p-6 border-b border-gray-100">
                    <h2 className="text-xl font-bold text-gray-800">관리자 추가</h2>
                ...
                <div className="px-6 pt-4">
                    <div className="flex border-b border-gray-200">
                        <button
                            className={`py-2 px-4 text-sm font-medium border-b-2 transition ${mode === 'create' ? 'border-black text-black' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
                            onClick={() => setMode('create')}
                        >
                            새 계정 생성
                        </button>
                        <button
                            className={`py-2 px-4 text-sm font-medium border-b-2 transition ${mode === 'select' ? 'border-black text-black' : 'border-transparent text-gray-500 hover:text-gray-700'}`}
                ...
                <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6">
                    {mode === 'select' && !selectedExistingUser ? (
                                        ...
                                        {filteredExistingUsers.map(u => (
                                            ...
                                            >
                                                <div>
                                                    ...
                                                    <div className="text-xs text-gray-500">{u.username} {u.displayName && `(${u.displayName})`}</div>
                                                </div>
                                                <div className="text-xs bg-gray-100 px-2 py-1 rounded text-gray-600">선택</div>
                                            </li>
                                        ))}
                                    </ul>
                                )}
                        ...
                        <div className="space-y-4">
                            {mode === 'select' && selectedExistingUser && (
                                <div className="bg-blue-50 p-4 rounded-md mb-4 flex justify-between items-center">
                                    <div>
                                        <p className="text-sm font-bold text-blue-900">선택된 사용자</p>
                                        <p className="text-sm text-blue-800">{selectedExistingUser.email}</p>
                                    </div>
                                    <button
                                        type="button"
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => {
                                            const formatted = e.target.value.replace(/[^\d]/g, '').replace(/^(\d{0,3})(\d{0,4})(\d{0,4})$/g, "$1-$2-$3").replace(/(\-{1,2})$/g, "");
                    ...
                    <div className="flex gap-4 mt-6 pt-6 border-t border-gray-200">
                        ...
                        <button
                            type="submit"
                            disabled={loading || (mode === 'select' && !selectedExistingUser)}
                        ...
                        >
                            {loading ? '처리 중...' : (mode === 'create' ? '생성' : '승격')}
                        </button>
                    </div>
                </form>
            </div>
